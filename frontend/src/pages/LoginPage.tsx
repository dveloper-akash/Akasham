    ...
    <div className='p-5  flex flex-col items-center h-screen'>
      ...
      <div className="w-full mt-25  px-5 max-w-150   flex justify-center ">
        <form onSubmit={handleSubmit} className="flex  flex-col bg-black text-white  w-full  items-center  px-10 pt-9 rounded-3xl ">
          ...
          <div className='w-full tracking-wide text-sm items-center opacity-80 py-1 px-2  flex justify-end'>
            <button onClick={handleForgotPassword} type='button'className='cursor-pointer hover:text-blue-500' >Forget Password</button>
          </div>
          <button type='submit' disabled={loading}
            className={`${loading ? "opacity-70" : ""
              } w-full  text-black bg-white font-bold flex border border-transparent hover:border-white hover:bg-transparent hover:text-white transition duration-200  rounded-full mt-3 py-2  cursor-pointer    items-center justify-center  tracking-wide text-xl`}>{loading ? (
                <svg
              ...
              <h2 className='pt-4 text-sm  pb-2'>dont have an account? <Link to="/signup" className='text-blue-400'>Signup</Link></h2>
